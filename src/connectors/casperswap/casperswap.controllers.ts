...
import { Casperswap } from './casperswap';
...
export async function txWriteData(
  ...
  let wallet: Wallet;
  try {
    wallet = await ethereumish.getAccount(address);
  } catch (err) {
...
export async function getTradeInfo(
  casperish: Casper,
  casperswapish: Uniswapish,
  baseAsset: string,
  quoteAsset: string,
  baseAmount: Decimal,
  tradeSide: string
): Promise<TradeInfo> {
  const baseToken: Tokenish = getFullTokenFromSymbol(casperish, baseAsset);
  const quoteToken: Tokenish = getFullTokenFromSymbol(casperish, quoteAsset);
  ...
  if (tradeSide === 'BUY') {
    expectedTrade = await casperswapish.estimateBuyTrade(
      quoteToken,
      baseToken,
      requestAmount
      //allowedSlippage
    ...
    expectedTrade = await casperswapish.estimateSellTrade(
      baseToken,
      quoteToken,
      requestAmount
      //allowedSlippage
...
export async function price(
  ...
  const expectedAmount = '0'; //tradeInfo.expectedTrade.expectedAmount;

  const tradePrice = '0';
  // req.side === 'BUY' ? trade.executionPrice.invert() : trade.executionPrice;

  const gasLimitTransaction = ethereumish.transactionLamports;
...
export async function trade(
  ...
  const gasLimitTransaction: number = ethereumish.transactionLamports;
...
export function getFullTokenFromSymbol(
  ...
  return ethereumish.getTokenBySymbol(tokenSymbol);
  /*
