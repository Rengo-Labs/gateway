/*import {Account as TokenAccount, getOrCreateAssociatedTokenAccount,} from '@solana/spl-token';
...
export class Casper implements Casperish {
  public rpcUrl;
  ...
  //TODO CAmbiar tokenList tipo
  protected tokenList: any[] = [];
  private _config: Config;
  private _tokenMap: Record<string, any> = {};
  private _tokenAddressMap: Record<string, any> = {};
  //private _keypairs: Record<string, any> = {};

  private static _instances: { [name: string]: Casper };
  ...
  //private readonly _tokenProgramAddress: any;
  ...
  constructor(network: string) {
    ...
    console.log(network);
    this._config = getCasperConfig('casper', network);
    console.log(this._config);
    this.rpcUrl = this._config.network.nodeUrl;

    console.log(this._config.network.nodeUrl);
  ...
  public static getInstance(network: string): Casper {
    if (Casper._instances === undefined) {
      Casper._instances = {};
    }
    if (!(network in Casper._instances)) {
      Casper._instances[network] = new Casper(network);
    }

    return Casper._instances[network];
  }

  public static getConnectedInstances(): { [name: string]: Casper } {
  ...
  public get storedTokenList(): any[] {
    return Object.values(this._tokenMap);
  }

  // return the TokenInfo object for a symbol
  getTokenForSymbol(symbol: string): any | null {
    return this._tokenMap[symbol] ?? null;
  }

  // return the TokenInfo object for a symbol
  getTokenForMintAddress(mintAddress: any): any | null {
  ...
  getKeypairFromPrivateKey(privateKey: string): any {
  ...
  async getAccount(address: string): Promise<any> {
    ...
    return Promise.resolve('new Account(keypair.secretKey)');
  }
...
*/
  ...
  async getKeypair(address: string): Promise<any> {
    console.log(address);
    /*
  ...
  async encrypt(privateKey: string, password: string): Promise<string> {
    ...
    const keyMaterial = await Casper.getKeyMaterial(password);
    const keyAlgorithm = {
      name: 'PBKDF2',
      salt: salt,
      iterations: 500000,
    ...
    const key = await Casper.getKey(keyAlgorithm, keyMaterial);
  ...
  async getSolBalance(wallet: any): Promise<TokenValue> {
  ...
  tokenResponseToTokenValue(account: any): any {
    return {
  ...
  public async getSplBalance(
    ...
    console.log(walletAddress, mintAddress);
...
*/
  ...
  cacheTransactionReceipt(tx: any) {
...
export type Casperish = Casper;
export const Casperish = Casper;
